    ...
    (async () => {
      ...
      try {
        ...
        if (!snap.exists()) {
          ...
          if (tgUser.photo_url && tgUser.photo_url !== data.photo) updates.photo = tgUser.photo_url;
          ...
          setUserData({ ...data, coins: data.coins });
        }
        ...
        {screen==='convert' && (
          <motion.div key="convert" variants={pv} initial="initial" animate="animate" exit="exit" className="flex-1 overflow-y-auto no-scrollbar flex flex-col">
            ...
            <div className="p-5 flex-1 flex flex-col gap-4">
              ...
              <p className="text-zinc-500 text-[10px] font-black uppercase tracking-widest px-1">পরিমাণ বেছে নাও</p>
                ...
                {[1,2,5,10].map(mult => {
                  ...
                  return (
                    <motion.button key={mult} whileTap={{scale:0.95}}
                      ...
                      className={`py-3 px-2 rounded-[16px] border text-center transition-all ${sel?'bg-amber-500/20 border-amber-500/50':has?'bg-[#1a1a1d] border-white/5 hover:border-white/10':'bg-[#111] border-white/3 opacity-40'}`}>
              ...
              {convertedTaka && (
                <motion.div initial={{opacity:0,scale:0.95}} animate={{opacity:1,scale:1}}
                  className="bg-emerald-500/10 border border-emerald-500/20 rounded-[20px] p-4 flex items-center justify-between">
                  <span className="text-zinc-400">{parseInt(convertCoins).toLocaleString()} Coin</span>
                  <Sparkles size={16} className="text-amber-400" />
                  <span className="text-emerald-400 font-black text-xl">৳{convertedTaka}</span>
                </motion.div>
              )}

              <motion.button whileTap={convertedTaka?{scale:0.98}:{}} onClick={handleConvert} disabled={convertLoading||!convertedTaka}
                className={`w-full py-4 rounded-[20px] font-black text-lg flex items-center justify-center gap-2 mt-auto ${convertedTaka?'bg-white text-black':'bg-[#1a1a1d] text-zinc-600 opacity-60'}`}>
